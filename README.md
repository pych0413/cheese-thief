...
| 📳 **搖部機擲骰** | 真係攞住部機搖就等於搖骰（iPhone 要㩒一下批准動作權限）。鎖咗點數就搖極都唔會變。 |
...
骰盅嗰個唔係遮住你對眼，係封住個重搖掣（連搖部機都封埋）：大家搖完各自鎖定，就冇人可以偷偷搖多次搵個靚啲嘅數。
...
骰盅下面有粒 **📳 搖骰** 掣。開咗之後真係攞住部機搖就會擲骰，唔使㩒螢幕。

- **iPhone（iOS 13+）**：第一次要㩒嗰粒掣，Safari 會彈個「動作與方向」權限出嚟，㩒「允許」。
  一定要經由㩒掣去問 — 呢個係 Apple 嘅規定，自動彈唔到。
  唔小心㩒咗「不允許」嘅話 Safari 會記住，要喺網址列㩒「ㄅA」→ 網站設定 開返，或者清除本站資料再試。
- 要 **HTTPS** 先用得（GitHub Pages 本身就係 HTTPS，冇問題）。
- **鎖咗點數就唔會再搖**，搖極個盅只會震一震同彈個提示。
- 唔想用（部機放袋、撞到枱）就㩒多次熄咗佢，設定會記住。

判定：連續 **3 下**超過 `10 m/s²` 嘅震動、喺 **1 秒**內完成先當一次搖骰，擲完靜 1.5 秒。
放低部機嗰下只係一下震動，唔會誤觸。覺得太靈敏／唔夠靈敏，改 `js/shake.js` 個 `threshold` 就得。

> 順帶一提：iPhone 嘅 Safari **冇** `navigator.vibrate`，所以震動回饋喺 iPhone 上面唔會出，
> 靠個骰盅嘅動畫做回饋。Android 就有。
...
js/shake.js         加速度計搖骰偵測（含 iOS 權限處理）
