...
| 🔊 **聲效** | 骰仔喺盅入面碌、揭牌、上鎖、開牌全部有聲，即場合成，唔使download任何音檔。右上角㩒一下靜音。 |
...
全部用 Web Audio 即場合成，**冇任何音檔**要download — 骰仔碰撞係一串濾過嘅雜訊，鈴聲係振盪器。

| 聲 | 幾時出 |
|---|---|
| 骰仔碌盅 | 自己搖、搖部機、或者主持叫全體搖骰 |
| 掀盅 / 揭牌 | 㩒住睇嗰下 |
| 上鎖 / 解鎖 | 㩒鎖定掣 |
| 低沉「唔得」 | 㩒鎖咗嘅角色牌，或者搖鎖咗嘅骰 |
| 派牌 | 每次派牌 / 下一回合 |
| 開牌和弦 | 主持開晒角色 |
| 叮 | 有人入房 |
...
1. **冇震動。** iPhone 部機梗係震得，但 **Safari 根本冇實作 Web Vibration API**（`navigator.vibrate`），
   網頁係無論如何都震唔到 iPhone 嘅 — 呢個係 WebKit 嘅決定，唔係我哋做漏咗。
   所以 iPhone 上面全部回饋靠**聲同動畫**。Android Chrome 就有震。
2. **Safari 嘅 Web Audio 跟實體靜音掣走。** 部機扳咗靜音就聽唔到聲。
   想聽就閂返靜音掣（其實咁樣先啱 — 唔想嘈嘅時候就唔應該出聲）。
